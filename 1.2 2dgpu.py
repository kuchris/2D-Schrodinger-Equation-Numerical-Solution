...
import os
my_path = os.path.abspath(r"G:\My Drive\Project\2D Schrodinger Equation Numerical Solution") # Figures out the absolute path for you in case your working directory moves around.
...
plt.xlabel(r'$X$')
plt.ylabel(r'$Y$')
plt.savefig(os.path.join(my_path, 'Figure_{}.0.pdf'.format(n)))
##plot eigenvector
...
plt.xlabel(r'$X$')
plt.ylabel(r'$Y$')
plt.savefig(os.path.join(my_path, 'Figure_{}.1.pdf'.format(n)))

##plot probability density
plot2 = plt.figure(2, figsize=(8,6))
cs = plt.contourf(X,Y, get_e(n)**2,100)
plt.colorbar()
for c in cs.collections:
    c.set_rasterized(True)
...
plt.xlabel(r'$X$')
plt.ylabel(r'$Y$')
plt.savefig(os.path.join(my_path, 'Figure_{}.2.pdf'.format(n)))
##plot eigenvalues
plot3 = plt.figure(3)
